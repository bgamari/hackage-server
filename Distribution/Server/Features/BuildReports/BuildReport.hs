...
import Distribution.FieldGrammar
         ...
         , uniqueField, uniqueFieldAla, booleanFieldDef, monoidalFieldAla, monoidalField )
...
initialBuildReport = BuildReport {
    _package         = requiredField "package",
    _time            = Nothing,
    _docBuilder      = False,
    _os              = requiredField "os",
    _arch            = requiredField "arch",
    _compiler        = requiredField "compiler",
    _client          = requiredField "client",
    _flagAssignment  = [],
    _dependencies    = [],
    _installOutcome  = requiredField "install-outcome",
--    _cabalVersion  = Nothing,
--    _tools         = [],
    _docsOutcome     = NotTried,
    _testsOutcome    = NotTried
  }
  where
...
parseFields input = do
  ...
  case partitionFields fields of
    ...
    otherwise -> fail "found sections in BuildReport"
